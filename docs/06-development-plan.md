...
현재는 1~3 중 운영자 자율 신청, 검증 상태·시도 이력, 가짜/수동 검증 어댑터와 재확인 API만 구현한다. 비로그인 첫 화면에는 메이트 이용·운영자 등록 선택을 두고, 운영자 선택은 같은 카카오 인증 뒤 일반 테니스 프로필 없이 OP01으로 연결한다. 이 선택은 계정·영구 역할·권한을 새로 저장하지 않는다. 코트 초안 저장, 증빙 업로드, 내부 검토 화면과 실제 외부 제공자 호출은 포함하지 않는다. 이 제한은 운영 권한을 추측해 공개하거나 사업자 원문을 일반 저장소에 남기지 않기 위한 것이다.
