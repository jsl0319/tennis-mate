...
현재는 1~3 중 운영자 자율 신청, 검증 상태·시도 이력, 가짜/수동 검증 어댑터와 재확인 API만 구현한다. 코트 초안 저장, 증빙 업로드, 내부 검토 화면과 실제 외부 제공자 호출은 포함하지 않는다. 이 제한은 운영 권한을 추측해 공개하거나 사업자 원문을 일반 저장소에 남기지 않기 위한 것이다.
