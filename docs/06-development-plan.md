...
6. 재확인·공개 중지·연결된 세션 안내 운영 절차 구현
...
11. CP01 제휴 코트 세션 탐색과 CP02 참가 신청 흐름
12. 기존 모집자의 참가 수락·거절 권한 재사용 검증
13. 세션 취소와 Slot 공개 상태 보존·재개·운영자 취소 영향 정책 검증
14. 운영자 제공 대표 코트 사진과 사진 없는 기본 일러스트 표시 — 외부 예약 사진에서 검증한 비공개 저장·JPEG/PNG/WebP 시그니처 검사·미연결 24시간 정리 경계를 재사용하고, 운영자 사진의 보관 정책을 별도 확정한 뒤

### 23.1.1 현재 구현 단위

현재는 1~4, 7~10의 최소 서버 수직 단위를 구현한다. 운영자 자율 신청, 검증 상태·시도 이력, 가짜/수동 검증 어댑터와 재확인 API에 이어 `DRAFT_ACCESS_GRANTED` 이상 운영자의 비공개 Court·Slot 초안, `PUBLISH_APPROVED`의 공개·중지, 일반 회원용 공개 Slot 조회와 `AVAILABLE → ALLOCATED` 제휴 코트 Match 원자적 연결을 포함한다. 비로그인 첫 화면에는 메이트 이용·운영자 등록 선택을 두고, 운영자 선택은 같은 카카오 인증 뒤 일반 테니스 프로필 없이 OP01으로 연결한다. 이 선택은 계정·영구 역할·권한을 새로 저장하지 않는다. CourtUnit 별도 관리 UI, 증빙 업로드, 내부 검토 화면, 운영자 사진, 실제 외부 제공자 호출, CP01/CP02 UI와 결제는 포함하지 않는다. 이 제한은 운영 권한을 추측해 공개하거나 사업자 원문을 일반 저장소에 남기지 않기 위한 것이다.
