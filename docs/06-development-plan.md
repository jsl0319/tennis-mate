...
16. 운영자 제공 대표 코트 사진과 사진 없는 기본 일러스트 표시 — 외부 예약 사진에서 검증한 비공개 저장·JPEG/PNG/WebP 시그니처 검사·미연결 24시간 정리 경계를 재사용하고, 운영자 사진의 보관 정책을 별도 확정한 뒤

### 23.1.1 현재 구현 단위

현재는 1~15의 운영자·공급 보호·공개 Slot 조회 단위와 CP01 공개 Slot 목록·상세, CP03 `AVAILABLE → ALLOCATED` 세션 개설 UI가 구현되어 있다. CP02는 기존 Match 상세·신청 흐름을 재사용한다. 모집자 취소가 연결 Slot을 자동 재공개하지 않고, 운영자 확인 뒤 `ALLOCATED → BLOCKED`와 새 `DRAFT`로만 재공급되는 경로도 포함한다. 또한 `INTERNAL_REVIEWER` 역할, 대기 신청 목록, 승인·보완·반려 판정 API와 변경 불가 감사 이력이 구현되어 실제 운영자가 `REVIEW_REQUIRED`에서 안전하게 공개 승인으로 갈 수 있다. 사업자등록증은 모든 운영자 신청의 필수 비공개 증빙으로 업로드·서버 서명 검사·원자적 연결·심사자 전용 열람·보관 기한 삭제를 구현한다. 역할 부여 UI·실제 외부 검증 호출·조건부 운영 권한 보완 증빙·운영자 사진과 결제는 이 다음 단위에 남긴다. 2026-08-28에는 사용자가 Figma 사용량 한도로 소스 선구현을 예외 승인했으므로, Figma 쓰기·동기화는 수행하지 않는다. 공개 Slot·연결 Match는 운영자 사진 업로드 전까지 기존 기본 코트 일러스트를 표시한다. 이 제한은 운영 권한을 추측해 공개하거나 사업자 원문을 일반 저장소에 남기지 않기 위한 것이다.
