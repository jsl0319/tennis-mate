...
16. 운영자 제공 대표 코트 사진과 사진 없는 기본 일러스트 표시 — 공개 승인 운영자만 최대 3장, JPEG/PNG/WebP·10 MiB·서버 시그니처 검사, 대표 1장 보호 중계, 미연결 24시간·제거/교체 즉시 정리와 향후 비활성화 30일 보관 경계

### 23.1.1 현재 구현 단위

현재는 1~16의 운영자·공급 보호·공개 Slot 조회 단위와 CP01 공개 Slot 목록·상세, CP03 `AVAILABLE → ALLOCATED` 세션 개설 UI가 구현되어 있다. CP02는 기존 Match 상세·신청 흐름을 재사용한다. 모집자 취소가 연결 Slot을 자동 재공개하지 않고, 운영자 확인 뒤 `ALLOCATED → BLOCKED`와 새 `DRAFT`로만 재공급되는 경로도 포함한다. 또한 `INTERNAL_REVIEWER` 역할, 대기 신청 목록, 승인·보완·반려 판정 API와 변경 불가 감사 이력이 구현되어 실제 운영자가 `REVIEW_REQUIRED`에서 안전하게 공개 승인으로 갈 수 있다. 사업자등록증은 모든 운영자 신청의 필수 비공개 증빙으로 업로드·서버 서명 검사·원자적 연결·심사자 전용 열람·보관 기한 삭제를 구현한다. 대표 코트 사진은 공개 승인 운영자가 최대 3장을 관리하고, 대표 1장을 공개 Slot과 연결 Match에서 보호된 중계 경로로 표시한다. 미연결 사진은 24시간, 제거·교체 사진은 즉시 정리하며, Court 비활성화·승인 취소 전환이 추가될 때 30일 보관을 적용할 구조를 둔다. 역할 부여 UI·실제 외부 검증 호출·조건부 운영 권한 보완 증빙·Court 비활성화 상태와 결제는 별도 단위다. 2026-08-28에는 사용자가 Figma 사용량 한도로 소스 선구현을 예외 승인했으므로, Figma 쓰기·동기화는 수행하지 않는다.
...
- 대표 코트 사진은 공개 승인 운영자·자신의 Court·검증된 JPEG/PNG/WebP 10 MiB·최대 3장 규칙을 모두 통과해야 연결되며, 일반 이용자는 대표 1장만 보호된 중계 경로로 본다. 미연결 24시간·제거/교체 즉시 정리와 비활성화 30일 만료 후보 정리를 검증한다.
