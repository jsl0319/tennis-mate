...
| D-03 | 매칭 후 연락 | 현재 Core는 매칭별 카카오 오픈채팅 링크를 모집자와 수락자에게만 공개. Match Chat 단계의 새 Match는 같은 멤버십의 서비스 내 텍스트 방 사용 |
...
서비스 내 Match Chat의 설계 기준은 `08-in-app-match-chat-design.md`에 있다. 현재 카카오 오픈채팅 Core 흐름을 교체하거나 실시간 인프라를 추가하기 전에 다음을 확인한다.

- 메시지·신고 보관·파기, 약관·개인정보 처리 문구와 신고 처리 SLA를 확정한다.
- `INTERNAL_REVIEWER`의 메시지 숨김·발신 중지·방 읽기 전용 조치와 감사 범위를 검토한다.
- 기존 `KAKAO_OPEN_CHAT` Match를 계속 읽는 호환 기간과 새 `IN_APP_CHAT` Match의 생성 규칙을 확정한다.
- PostgreSQL 폴링 부하와 사용자 체감 지연을 Preview에서 측정한다. WebSocket·Redis·외부 실시간 서비스는 이 측정과 별도 승인 없이는 도입하지 않는다.

권장 구현 순서는 다음과 같다.

1. `MatchContactMode`, Conversation·Member·Message·Report·ModerationAction의 migration과 메시지 보관 작업을 설계한다.
2. 수락 트랜잭션의 방 생성·멤버 추가와 Match 취소·공급 철회·이용 종료의 읽기 전용 전환을 구현한다.
3. 커서 기반 조회, `clientRequestId` 멱등 발신, 읽음 커서와 활성 화면 5초 폴링 UI를 구현한다.
