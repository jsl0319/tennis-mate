...
Court Partner Pilot은 다음 신호가 확인될 때 검토한다.

- 사용자는 매칭 의사가 있지만 직접 코트 예약 때문에 등록하지 못한다.
- 외부 예약 Match의 참가 신청과 수락이 반복적으로 발생한다.
- 코트 운영자가 유휴 시간대를 직접 공급할 의사가 있다.
- 예약 승인 업무를 수동 운영으로 먼저 검증할 수 있다.

단순 가입자 수만으로 Partner 기능을 시작하지 않는다.
...
Core MVP 사용자 테스트에서 필요성이 확인된 뒤 새 계획으로 분리한다.

### 23.1 Pilot 순서

1. 운영자 신청과 수동 승인
2. Court·CourtUnit 관리
3. CourtSlot 등록과 겹침 방지
4. 사용자 예약 가능 Slot 조회
5. CourtBooking 요청
6. 운영자 승인·거절
7. CONFIRMED Booking 기반 Match 생성
8. 예약 취소와 Match 영향 정책 검증
