...
13. CP01 제휴 코트 세션 탐색과 CP02 참가 신청 흐름
...
현재는 1~4, 7~10의 최소 서버 수직 단위가 구현되어 있다. 다음 수직 단위는 먼저 공개 Slot 불변·공개 중지·공급 철회 정책을 문서와 Figma에 반영하고, `DRAFT` 전용 수정·운영자 Slot 목록·공급 Incident·대상 인앱 안내·반복 철회 새 공개 제한을 서버와 테스트로 구현한다. 그 다음 OP03~OP06 운영자 시간 관리 UI를 구현한다. 비로그인 첫 화면에는 메이트 이용·운영자 등록 선택을 두고, 운영자 선택은 같은 카카오 인증 뒤 일반 테니스 프로필 없이 OP01으로 연결한다. 이 선택은 계정·영구 역할·권한을 새로 저장하지 않는다. CourtUnit 별도 관리 UI, 증빙 업로드, 내부 검토 화면, 운영자 사진, 실제 외부 제공자 호출, CP01/CP02 UI와 결제는 이 다음 단위에 남긴다. 이 제한은 운영 권한을 추측해 공개하거나 사업자 원문을 일반 저장소에 남기지 않기 위한 것이다.
