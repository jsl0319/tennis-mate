...
현재는 1~13의 운영자·공급 보호·공개 Slot 조회 단위와 CP01 공개 Slot 목록·상세, CP03 `AVAILABLE → ALLOCATED` 세션 개설 UI가 구현되어 있다. CP02는 기존 Match 상세·신청 흐름을 재사용한다. 2026-08-28에는 사용자가 Figma 사용량 한도로 소스 선구현을 예외 승인했으므로, Figma 쓰기·동기화는 수행하지 않는다. 공개 Slot·연결 Match는 운영자 사진 업로드 전까지 기존 기본 코트 일러스트를 표시한다. CourtUnit 별도 관리 UI, 증빙 업로드, 내부 검토 화면, 운영자 사진, 실제 외부 제공자 호출과 결제는 이 다음 단위에 남긴다. 이 제한은 운영 권한을 추측해 공개하거나 사업자 원문을 일반 저장소에 남기지 않기 위한 것이다.
