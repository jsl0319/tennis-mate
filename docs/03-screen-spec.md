...
- 예상 1인 비용을 `ceil(전체 비용 ÷ 예상 총 참여 인원)`으로 자동 계산한다.
...
수락자가 있으면 모집자에게만 채팅방 CTA를 제공한다. 새 `EXTERNAL_RESERVED` Match에는 코트를 다시 정하는 것으로 보이지 않도록 `당일 준비와 비용 정산 방법을 확인해요`라고 안내한다. 과거 `COURT_TBD` 기록은 기존 참여자가 남은 조율과 완료 처리를 할 수 있도록 이전 안내를 유지한다. 어떤 경우에도 Tennis Mate가 비용을 결제하거나 정산하지 않는다는 책임 범위를 유지한다.
