...
- 향후 제휴 코트 세션: `Tennis Mate에서 준비한 코트예요`
...
| `PARTNER_COURT` | Tennis Mate에서 준비한 코트예요 | 운영자가 준비한 시간에 열린 세션이에요. 참가 신청은 모집자에게 보내요. | 향후 별도 강조 색상 및 확인 아이콘 |
...
| 단계 | 제목 | 주요 입력 |
| --- | --- | --- |
| 1 | 언제, 어디서 칠까요? | 날짜, 시작 시간, 이용 시간, 지역, 예약 상태, 예약 코트일 때만 코트명·주소·코트 번호·대표 코트 사진 |
...
- 지역: 시·구 선택
- 예약 코트일 때만 코트장 이름, 주소, 코트 번호—선택, 대표 코트 사진 1장—선택
...
| 홈 | 하드코트 라인·테니스볼 랠리 로더 | 추천 없음과 전체 없음 구분 | 재시도 및 캐시 데이터 유지 |
...
현재 Core 화면의 카카오 오픈채팅 CTA를 즉시 바꾸지 않는다. 새 `IN_APP_CHAT` Match가 승인되어 구현될 때, S04 상세와 S07 보낸 신청은 모집자·`ACCEPTED` 참가자에게만 `채팅방 열기`를 보인다. `PENDING`, 거절·철회·수락된 적 없는 취소 신청자, 운영자와 제3자에게는 방 존재·멤버를 노출하지 않는다. Match 취소 뒤에는 이미 입장한 멤버만 안전한 안내를 읽기 전용으로 본다.

| 상태 | 화면 행동 |
| --- | --- |
| 첫 수락 전 | 채팅 CTA를 보이지 않음 |
| `OPEN` | Match 제목·일정 요약·텍스트 메시지·신고·전송 입력 제공 |
| `READ_ONLY` | 취소 또는 이용 종료 안내와 기존 메시지만 제공. 입력과 재개 CTA는 숨김 |
| `ARCHIVED` | 활동 목록에서 제거하고 일반 메시지에 접근시키지 않음 |

방은 개인 메신저가 아니므로 사용자 검색, DM, 사진·파일, 읽음·접속·입력 중 표시를 추가하지 않는다. 모바일 키보드·안전 영역, 로딩·빈 메시지·폴링 실패·신고 접수 상태는 `08-in-app-match-chat-design.md`를 기준으로 별도 설계한다. Figma는 사용량 한도가 해제된 뒤 사용자가 명시적으로 요청할 때만 동기화한다.
