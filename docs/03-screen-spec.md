...
하단 메뉴 아이콘은 채워진 28px, 레이블은 14px, 각 항목의 최소 터치 높이는 72px로 둔다. `매칭`은 함께 칠 상대를 찾는 의미의 채워진 두 사람 아이콘을 사용하고, 선택 상태는 하드코트 블루와 레이블로 함께 구분한다. `코트 매칭`은 채워진 코트 아이콘, `채팅`과 `마이`는 같은 무게의 채워진 아이콘을 사용해 네 항목의 시각적 존재감을 맞춘다.
