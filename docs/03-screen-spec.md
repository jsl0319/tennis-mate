...
| S05 | 매칭 등록 | `/matches/new` | 외부 예약 코트로 참가자 모집 |
...
| `NOT_RESERVED` | 코트는 아직 정해지지 않았어요 | Core MVP 공개 매칭에서는 사용하지 않음 | 경고가 아닌 정보 스타일 |
...
| 상태 | 화면 처리 | CTA |
| --- | --- | --- |
| 추천 있음 | 추천 3~5개와 다른 매칭 표시 | 상세 보기 |
| 추천 없음·다른 매칭 있음 | `조건이 꼭 맞는 매칭은 아직 없어요` 후 다른 매칭 표시 | 둘러보기 |
...
> 아직 가까운 매칭이 없어요.  
> 이미 예약한 코트가 있다면 먼저 편하게 모집해볼 수 있어요.
...
외부에서 코트를 이미 예약한 사용자가 짧고 명확한 단계로 참가자 모집을 등록하게 한다.
...
| --- | --- |
| `이미 예약했어요` | 등록 1단계로 이동 |
| `아직 예약하지 않았어요` | Core MVP에서는 공개 등록 불가 안내 |
| `제휴 코트를 예약하고 싶어요` | 향후 기능 안내. Core MVP에서는 노출하지 않거나 비활성 |

권장 안내:

> 지금은 이미 예약된 코트의 참가자 모집부터 지원하고 있어요. 제휴 코트 예약은 준비 중이에요.

Court Partner Pilot이 활성화되면 `제휴 코트를 예약하고 싶어요`를 선택하여 `03-1-court-partner-screen-spec.md`의 CT01 예약 가능 코트 화면으로 이동한다.

### 12.3 단계 구조

| 단계 | 제목 | 주요 입력 |
| --- | --- | --- |
| 1 | 언제, 어디서 치나요? | 날짜, 시작 시간, 이용 시간, 지역, 코트명, 주소, 코트 번호 |
| 2 | 어떤 테니스를 함께할까요? | 제목, 모집 인원, 원하는 플레이, 상대 수준—초보자 환영 선택 포함 |
| 3 | 비용과 연락 방법을 알려주세요 | 전체 비용, 예상 1인 비용, 추가 비용, 소개 메시지, 카카오 오픈채팅 링크 |
...
- 예상 총 참여 인원: 모집자 포함
- 예상 1인 비용: 기본 자동 계산
- 추가 비용 안내: 조명비·볼 비용 포함 여부
- 소개 메시지: 선택
- 카카오 오픈채팅 링크: 필수, 수락된 참가자에게만 공개

권장 UX안:

- 예상 1인 비용은 `ceil(전체 비용 ÷ 예상 총 참여 인원)`으로 자동 계산한다.
...
| 상태 | 처리 |
| --- | --- |
| 만든 매칭 없음 | 외부 예약 코트가 있다면 매칭 등록 안내 |
...
- [ ] Core MVP에서는 외부 예약 코트 등록 흐름만 활성화된다.
