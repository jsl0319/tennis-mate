...
하단 메뉴 아이콘은 24px, 레이블은 13px, 각 항목의 최소 터치 높이는 64px로 둔다. `매칭`은 함께 칠 상대를 찾는 의미의 교차 테니스 라켓 아이콘을 사용하고, 선택 상태는 하드코트 블루와 레이블로 함께 구분한다.
...
| `COURT_TBD` | 코트와 비용을 함께 정해요 | 수락된 참가자와 채팅에서 조율해요. | 경고가 아닌 정보 스타일 |
...
추천 목록과 다른 매칭 목록은 한 화면에 함께 쌓지 않는다. 기본 선택은 `추천 매칭`이며, 사용자가 선택 버튼으로 `다른 매칭`을 확인한다. `매칭 만들기`에서는 예약한 외부 코트 또는 `COURT_TBD`를 선택할 수 있다. `COURT_TBD`는 일정·활동 지역·모집 정보만으로 공개하며 코트와 비용은 수락 후 현재 연락 수단으로 조율한다.
...
수락자가 있으면 모집자에게만 채팅방 CTA를 제공한다. `COURT_TBD`는 `코트와 비용을 조율해요`라고 안내하고, `EXTERNAL_RESERVED`는 코트를 다시 정하는 것으로 보이지 않도록 `당일 준비와 비용 정산 방법을 확인해요`라고 안내한다. 두 경우 모두 Tennis Mate가 비용을 결제하거나 정산하지 않는다는 책임 범위를 유지한다.
...
수락된 카드의 연락 안내는 채팅방 CTA와 코트 출처 안내를 함께 보인다. `COURT_TBD`는 코트와 비용 조율을, `EXTERNAL_RESERVED`는 당일 준비와 참가자 간 비용 정산 방법 확인을 안내한다. 예약 완료 코트에 `코트와 비용을 함께 정해요` 문구를 사용하지 않는다.
