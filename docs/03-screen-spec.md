...
| 항목 | 내용 |
| --- | --- |
| 문서명 | Tennis Mate Core MVP Screen Specification |
| 문서 상태 | Draft v0.1 |
...
6. 직접 예약 코트를 Tennis Mate가 검증한 코트로 오해하지 않게 한다.
...
- 향후 제휴 예약 확인: `Tennis Mate에서 예약이 확인된 코트예요`

두 유형에 동일한 아이콘, 색상 또는 `인증` 문구를 사용하지 않는다.
...
| 항목 | UX안 | 상태 |
| --- | --- | --- |
| 로그인 방식 | 카카오 로그인 한 종류로 시작 | 확정 |
| 닉네임 | 소셜 표시명을 기본 제안하고 최초 진입에서 확인·수정 | 확정 |
| 비로그인 탐색 | 서비스 소개만 제공하고 목록·상세는 로그인 후 접근 | 확정 |
| 매칭 후 연락 | 모집자의 매칭별 카카오 오픈채팅 링크를 수락된 사용자에게만 공개 | 확정 |
| 성별·연령 | Core MVP 필수 수집 제외 | 확정 |
| 코트 미확정 매칭 | 일정·활동 지역·모집 정보와 오픈채팅 링크를 입력해 공개 등록 가능. 코트·비용은 수락된 참가자와 조율 | 확정 |
...
| ID | 화면 | 제안 경로 | 핵심 목적 |
| --- | --- | --- | --- |
| S01 | 로그인·회원가입 | `/login` | 가장 적은 입력으로 서비스 시작 |
...
| S05 | 매칭 등록 | `/matches/new` | 예약된 코트 또는 코트 미정 상태로 참가자 모집 |
...
| --- | --- | --- |
| 홈 | `/` | 항상 |
| 매칭 만들기 | `/matches/new` | 항상 |
| 활동 | `/activity/received` 또는 마지막 선택 탭 | 받은 신청이 있으면 숫자 배지 표시 |
| 마이 | `/my` | 항상 |
...
| `PARTNER_COURT` | Tennis Mate에서 예약이 확인된 코트예요 | 예약 상태를 앱에서 확인했어요. | 향후 별도 강조 색상 및 확인 아이콘 |
| `COURT_TBD` | 코트와 비용을 함께 정해요 | 수락된 참가자와 오픈채팅에서 조율해요. | 경고가 아닌 정보 스타일 |
...
서비스의 핵심 가치를 짧게 전달하고 최소 행동으로 가입 또는 로그인하게 한다.

### 8.2 진입과 이탈

| 구분 | 내용 |
| --- | --- |
| 진입 | 비로그인 상태에서 인증 필요 화면 접근, 직접 URL 접근 |
| 성공 | 신규 사용자 → 닉네임 확인 후 S02, 온보딩 완료 사용자 → S03 |
| 이탈 | 브라우저 뒤로가기 또는 서비스 소개 확인 |

### 8.3 화면 구성 순서

1. Tennis Mate 로고와 한 줄 가치 제안
2. 초보자 중심 서비스임을 보여주는 짧은 설명
3. 카카오 로그인 CTA
4. 이용약관 및 개인정보 처리 안내
...
비로그인 사용자가 보호된 경로를 직접 열면 `/login?returnTo=...`으로 이동한다. `returnTo`는 동일 서비스의 상대 경로만 허용하며, 로그인 뒤 신규 사용자는 온보딩을 완료한 후, 기존 사용자는 곧바로 원래 경로로 돌아간다. 네트워크·서버 확인 실패는 로그인 실패와 구분해 `다시 확인하기` 상태로 보여준다.
...
- Primary CTA: `[카카오로 시작하기]`
- 로그인 실패 시 같은 화면에서 재시도
- 인증 진행 중 CTA 중복 클릭 방지
...
| --- | --- |
| 기본 | 가치 제안과 CTA 표시 |
| 인증 중 | CTA 로딩, 다른 인증 요청 차단 |
| 인증 실패 | 개인정보를 노출하지 않는 오류 문구와 재시도 |
| 이미 로그인 | 온보딩 여부에 따라 자동 이동 |
...
추천 목록과 다른 매칭 목록은 한 화면에 함께 쌓지 않는다. 기본 선택은 `추천 매칭`이며, 사용자가 선택 버튼으로 `다른 매칭`을 확인한다. `매칭 만들기`에서는 예약한 외부 코트 또는 `COURT_TBD`를 선택할 수 있다. `COURT_TBD`는 일정·활동 지역·모집 정보만으로 공개하며 코트와 비용은 수락 후 오픈채팅에서 조율한다.
...
ProfileSummaryCard를 사용한다. 연락처와 오픈채팅 링크는 신청 수락 전 노출하지 않는다.

수락된 신청자와 모집자에게만 `카카오 오픈채팅으로 연락하기` 영역을 표시한다. 링크 전체를 텍스트로 노출하지 않고 외부 이동임을 안내한다.
...
코트를 이미 예약한 사용자와 아직 코트를 정하지 못한 사용자가 모두 짧고 명확한 단계로 참가자 모집을 등록하게 한다.

첫 단계에서 `코트를 예약했어요`와 `코트는 같이 정해요`를 같은 규격의 선택 카드로 제공한다. 후자를 선택해도 등록을 막지 않으며, 일정·활동 지역만 먼저 입력한다.

### 12.3 단계 구조

| 단계 | 제목 | 주요 입력 |
| --- | --- | --- |
| 1 | 언제, 어디서 칠까요? | 날짜, 시작 시간, 이용 시간, 지역, 예약 상태, 예약 코트일 때만 코트명·주소·코트 번호 |
| 2 | 어떤 테니스를 함께할까요? | 제목, 모집 인원, 원하는 플레이, 상대 수준—초보자 환영 선택 포함 |
| 3 | 비용과 연락 방법을 알려주세요 | 예약 코트일 때만 전체 비용·예상 1인 비용·추가 비용, 소개 메시지, 카카오 오픈채팅 링크 |
| 4 | 이렇게 모집할까요? | 전체 미리보기와 등록 확인 |

각 단계는 상단 진행률과 뒤로가기를 제공한다. 이전 입력은 유지한다.
...
- 날짜: 오늘 이후만 선택
- 시작 시간
- 이용 시간: 선택형 1시간, 1시간 30분, 2시간, 직접 입력
- 지역: 시·구 선택
- 예약 코트일 때만 코트장 이름, 주소, 코트 번호—선택
...
- 예약 코트일 때만 전체 코트 비용(0원 허용), 예상 총 참여 인원, 예상 1인 비용, 추가 비용 안내
- 소개 메시지: 선택
- 카카오 오픈채팅 링크: 필수, 수락된 참가자에게만 공개

권장 UX안:

- 예약 코트일 때만 예상 1인 비용을 `ceil(전체 비용 ÷ 예상 총 참여 인원)`으로 자동 계산한다.
- 코트 미정일 때는 `코트와 비용은 함께 정해요` 안내만 보여주고 비용 입력과 계산값은 숨긴다.
- 계산값은 사용자가 수정할 수 없으며 추가 비용 안내에 차이를 설명한다.
- 오픈채팅 링크는 `https://open.kakao.com/` 주소만 허용하고 `수락된 참가자에게만 공개돼요`라고 안내한다.

필수 안내:

> Tennis Mate에서는 이 비용을 결제하지 않아요. 실제 비용은 참가자끼리 확인해 주세요.
...
- MatchCard 미리보기
- 일정과 코트
- `모집자가 코트를 예약했어요` 또는 `코트와 비용을 함께 정해요` 배지
- 예약 코트일 때 비용, 코트 미정일 때 비용 협의 안내
- 플레이 조건
- 내 프로필
- 소개 메시지
- 오픈채팅 링크 비공개 안내—링크 자체는 공개 미리보기에 표시하지 않음
...
받은 신청과 보낸 신청은 하단 전역 메뉴에서 진입하는 하나의 활동 화면이다. 두 목록 최상단에는 뒤로가기 버튼을 제공하지 않고 탭으로 전환한다. 매칭별 신청자 목록과 신청자 상세처럼 한 단계 들어간 화면에서는 뒤로가기를 제공한다.

### 13.3 매칭 요약 카드

- 제목
- 날짜·시간
- 코트 출처(`모집자가 코트를 예약했어요` 또는 `코트와 비용을 함께 정해요`)
- 모집 상태
- `수락 n명 / 모집 n명`
- `검토할 신청 n건`
- `[신청자 보기]`
...
수락자가 있으면 모집자에게만 카카오 오픈채팅 CTA를 제공한다. `COURT_TBD`는 `코트와 비용을 조율해요`라고 안내하고, `EXTERNAL_RESERVED`는 코트를 다시 정하는 것으로 보이지 않도록 `당일 준비와 비용 정산 방법을 확인해요`라고 안내한다. 두 경우 모두 Tennis Mate가 비용을 결제하거나 정산하지 않는다는 책임 범위를 유지한다.
...
최상단의 활동 탭 구조와 뒤로가기 원칙은 S06과 동일하게 적용한다.
...
수락된 카드의 오픈채팅 안내는 코트 출처에 따라 분기한다. `COURT_TBD`는 코트와 비용 조율을, `EXTERNAL_RESERVED`는 당일 준비와 참가자 간 비용 정산 방법 확인을 안내한다. 예약 완료 코트에 `코트와 비용을 함께 정해요` 문구를 사용하지 않는다.
...
| ACCEPTED | 같이 치게 됐어요 🎾 | 확정 정보 확인, 카카오 오픈채팅으로 연락 |
...
매칭 생성은 전역 하단 메뉴의 `[만들기]`에서 시작한다. 마이페이지에는 동일한 진입 CTA를 중복 제공하지 않는다.
...
| 온보딩 | 선택지 스켈레톤 최소화 | 해당 없음 | 선택 유지 후 재시도 |
| 홈 | MatchCard 스켈레톤 | 추천 없음과 전체 없음 구분 | 재시도 및 캐시 데이터 유지 |
| 상세 | 주요 섹션 스켈레톤 | 삭제·취소 상태 안내 | 홈 이동과 재시도 |
| 매칭 등록 | 제출 버튼 로딩 | 해당 없음 | 모든 입력 유지 |
| 받은 신청 | 매칭·신청 카드 스켈레톤 | 만든 매칭 없음과 신청 없음 구분 | 재시도 |
| 보낸 신청 | 신청 카드 스켈레톤 | 추천 매칭 CTA | 재시도 |
| 마이 | 프로필 카드 스켈레톤 | 온보딩 미완료 시 생성 CTA | 재시도 |
...
코트 운영자, 코트 시간대 등록, 예약 요청·승인, 사용자 예약 목록과 결제 화면은 `03-1-court-partner-screen-spec.md`에서 별도로 정의하며 Core MVP 화면 수에 포함하지 않는다.
