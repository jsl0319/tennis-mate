...
| S02 | 테니스 프로필 온보딩 | `/onboarding` | 초보자 언어로 플레이 상태 생성 |
...
온보딩, 매칭 등록, 매칭 상세의 집중 흐름에서는 필요에 따라 하단 내비게이션을 숨기고 상단 뒤로가기를 제공한다. 상단 뒤로가기는 홈으로 고정 이동하지 않고 브라우저 기록의 직전 화면으로 돌아가며, 이전 기록이 없는 직접 진입에서만 홈으로 이동한다.
...
추천 목록과 다른 매칭 목록은 한 화면에 함께 쌓지 않는다. 기본 선택은 `추천 매칭`이며, 사용자가 선택 버튼으로 `다른 매칭`을 확인한다. `매칭 만들기`는 예약 여부를 전제하지 않는 진입 문구지만, Core MVP 등록 과정에서는 이미 예약한 외부 코트 정보를 계속 필수로 받는다.
...
| CANCELLED | 매칭이 취소되거나 성사 없이 종료됐어요 | 다른 추천 매칭 보기 |

Match가 `CLOSED`이고 Application이 `CANCELLED`이면 `모집이 마감됐어요`라고 표시하여 취소·성사 없이 종료와 구분한다.
