...
2026-09-01의 공개 문서 기반 PG 사전 검증 결과와 계약 차단 항목은 `09-court-commerce-pg-preflight.md`에서 관리한다. 이 설계의 지급·환불·정산 구현 게이트는 해당 문서의 서면 계약·법무/세무·sandbox 검증 항목이 모두 통과한 뒤에만 닫힌다.
...
- Rally On은 한 세션의 결제·환불·공급 철회를 서로 모순 없이 기록해야 한다.
...
- Rally On의 수기 지급·수납·임시 자금 보관
- 운영자가 자신의 PG 키·계좌번호·결제 URL을 직접 입력하거나 참가자에게 공개하는 기능
- Rally On이 카드 정보나 계좌 정보를 저장하는 기능
...
| 주체 | 하는 일 | 하지 않는 일 |
| --- | --- | --- |
| 참가자 | 세션 상세에서 코트 총액·예상 부담·현장 분담 안내를 보고 신청·참가 | Rally On에 코트 비용을 결제하거나, 다른 사용자의 비용을 대신 보증 |
...
| Rally On | 단일 결제와 Slot·Match 상태를 연결하고 PG 결과를 검증·표시 | 참가자 간 돈 수납·환불 보장, 카드·계좌 원문 보관, 수기 정산 |
| PG/지급결제 제공자 | 모집자 결제 승인·취소·환불·운영자 정산 또는 하위가맹점 지급 | Match 참가자 승인 판단, 현장 비용 분담 판단 |

권장 계약 구조는 운영자가 PG의 판매자/하위가맹점으로 온보딩되고, Rally On이 플랫폼 중개자로 PG의 마켓플레이스·지급 기능을 사용하는 방식이다. 실제 계약상 판매자, 정산 주체, 세금계산서 및 환불 비용의 귀속은 선택한 PG 계약서와 법무·세무 검토로 확정한다. 이 문서는 법률·세무 자문이 아니다.
...
3. 운영자는 선택 PG의 사업자·정산 온보딩을 완료했다. 기존 사업자등록증은 Rally On의 운영자 심사 증빙이며 PG 심사를 대체하지 않는다.
4. Slot에 사용자 공개용 코트 이용 총액, 요금표 버전, 24시간 전 전액 취소 정책 버전이 고정되어 있다.
5. 지원 담당자, 공급 철회 환불 절차, PG 장애 안내가 운영 가능하다.

PG 온보딩에 필요한 정산 계좌 등 민감 정보는 PG의 전용 흐름에서만 수집한다. Rally On DB에는 제공자가 발급한 최소 식별자와 상태만 보관한다.

## 4. 확정된 상업 정책

### 4.1 코트 이용 총액과 요금표

유료 Slot의 가격 단위는 `1인 참가비`가 아니라 **코트 이용 총액**이다. 운영자는 초안에서 코트 면·이용 시간·현장 최대 인원과 함께 해당 시간의 사용자 공개 총액을 정한다. 참가자에게는 코트 이용 총액과 모집 인원을 기준으로 한 `예상 1인 부담`만 참고 정보로 보여 준다. 이 예상값은 Rally On의 청구·채권·수납 금액이 아니다.
...
| 참가자 비용 | 앱에서 청구하지 않음. 현장 자율 분담이며 Rally On이 금액·수납·환불을 보장하지 않음 |
...
- 테니스공은 `운영자 제공`, `모집자 준비`, `각자 준비` 중 하나를 사전에 표시한다. 공 비용·현장 추가금·개인 송금은 Rally On 결제 범위에 넣지 않는다.
- 참가 신청·수락·Match Chat은 결제 여부와 무관하게 기존 Match 규칙을 따른다. 참가자가 현장 비용을 내지 않았다는 이유만으로 Rally On이 참가 상태를 변경하거나 돈을 청구하지 않는다.
...
- 전액 환불은 원 결제수단의 PG 취소·환불로만 처리한다. Rally On이 계좌번호를 받아 수동 송금하지 않는다.
...
PG가 정산을 실제 지급하는 구조를 우선한다. Rally On이 수동 이체로 지급하거나 참가자 돈을 임시 보관하는 구조는 도입하지 않는다.

지급 요청은 모집자 결제 승인과 같은 순간에 만들지 않는다. 지급한 금액을 회수하기 어려운 제공자 제약을 고려해, 첫 출시에서는 `endsAt` 이후이고 공급 철회 Incident·환불 요청·미확정 상태가 없으며 제공자 지급 가능 잔액이 충분할 때만 운영자 지급을 요청한다. 실제 지급 보류 기간, 지급 후 환불·차지백 상계와 환수 책임은 PG 계약·법무/세무 검토로 확정한다. 운영자에게는 지급 전에도 `모집자 결제 완료` 상태를 안전하게 보여 줄 수 있다.
...
| Match 상세 | `Rally On에서 준비한 코트예요`, 모집자 결제 완료 상태, 예상 1인 부담은 참고값, `현장에서 함께 정산해요` | 참가자에게 결제 버튼·영수증·미납 표시 |
...
- [ ] PG 마켓플레이스/하위가맹점 계약에서 운영자·Rally On·PG의 단일 모집자 수납·환불·지급 책임을 확인했다.
