...
- 운영자 사진 업로드 뒤 저장을 완료하지 않은 `PENDING` 파일은 24시간 뒤 삭제한다. 운영자가 사진을 제거하거나 저장 과정에서 교체된 기존 사진은 즉시 공개에서 제외하고 다음 정리 작업에서 객체와 메타데이터를 삭제한다. Court 비활성화 또는 운영자 공개 승인 취소가 도입되면 연결된 사진은 30일 보관 뒤 삭제하도록 `expiresAt`을 설정한다. 현재 Pilot에는 Court 비활성화 전환 UI·API가 없으므로 그 전환 자체는 만들지 않는다.
