...
Court Partner Pilot과 분리한 다음 단계다. 일반 사용자 CourtBooking, 운영자의 참가 승인, Tennis Mate의 수기 수납을 만들지 않는다. 상세 설계 기준은 `07-court-commerce-design.md`다.

현재 확정된 제품 정책은 다음과 같다.

- 운영자가 PG의 판매자/하위가맹점 온보딩을 완료하고, PG의 마켓플레이스·지급 기능으로 결제·환불·정산을 처리하는 구조를 우선한다. 실제 계약상 책임은 PG 계약과 법무·세무 검토로 확정한다.
- 플랫폼 수수료는 운영자 부담 5%이며, PG 실수수료도 운영자가 부담한다.
- 운영자별 첫 성공 유료 세션 결제 **승인 시각**부터 30일간 플랫폼 수수료는 0%다. 가입일·세션 시작일이 기준이 아니며, PG 수수료는 무료 기간에도 면제하지 않는다.
- 참가자는 Slot 공개 전에 정해진 1인 고정 참가비만 본다. 기존 전체 코트 비용·예상 1인 비용을 실제 결제액으로 재사용하지 않는다.
- 모집자 결제 후에만 유료 세션을 확정하고, 참가 신청은 결제 초대의 승인 확인 뒤에만 `ACCEPTED`가 된다.
- 운영자 공급 철회로 Match를 취소하면 결제 완료 호스트·참가자에게 전액 환불한다. 모집자·참가자 취소, 우천, 노쇼는 구현 전에 별도 확정한다.

Commerce 구현은 PG 계약, 법무·세무 검토, 취소·환불 정책, 지원 절차, sandbox 검증이 끝난 뒤에만 시작한다.

### 3.4 서비스 내 Match 채팅

서비스 내 채팅은 현재 카카오 오픈채팅을 즉시 제거하는 Core 변경이 아니라, 별도 Match Chat 단계다. 범용 메신저·개인 DM·사진·파일·음성·읽음·접속·입력 중 표시는 제공하지 않는다. 상세 정책·보관·신고·전환 방식은 `08-in-app-match-chat-design.md`를 따른다.
...
- 신규 `IN_APP_CHAT` Match만 서비스 내 방을 사용하고, 기존 `KAKAO_OPEN_CHAT` Match는 호환 기간에 기존 링크를 유지한다.
...
11. 현재 Core에서는 모집자가 매칭별 카카오 오픈채팅 링크를 등록하고 모집자와 수락된 신청자에게만 공개한다. Match Chat 단계의 새 `IN_APP_CHAT` Match는 수락자 전용 서비스 내 방으로 연락한다.
...
| 카카오 오픈채팅 링크 | 현재 Core 필수 | 수락된 참가자에게만 공개할 매칭별 연락 링크. `IN_APP_CHAT` Match 출시 뒤 새 Match에는 요구하지 않음 |
...
- 현재 Core에서 수락된 신청자는 자신의 신청 목록에서 성사 상태와 카카오 오픈채팅 링크를 확인할 수 있다. `IN_APP_CHAT` Match에서는 같은 위치에서 채팅방 진입 상태를 확인한다.
...
- 전화번호와 이메일은 공개하지 않는다. 현재 Core의 매칭별 카카오 오픈채팅 링크 또는 `IN_APP_CHAT` Match의 방 상태만 모집자와 ACCEPTED 신청자에게 반환한다. 메시지·신고 원문은 로그·분석 이벤트에 넣지 않는다.
...
11. 현재 Core에서 모집자와 ACCEPTED 신청자만 카카오 오픈채팅 링크를 확인할 수 있다. `IN_APP_CHAT` 출시 뒤에는 같은 멤버십만 서비스 내 방에 입장할 수 있다.
...
| CORE-03 | 매칭 성사 후 연락 | 현재 Core는 매칭별 카카오 오픈채팅 링크를 모집자와 ACCEPTED 신청자에게만 공개. 이후 새 `IN_APP_CHAT` Match는 같은 멤버십의 서비스 내 텍스트 방 사용 |
