...
- 운영자의 참가자 선택·실력 심사
- 참가자별 좌석 판매 또는 분할 결제
- 범용 실시간 채팅, 개인 DM, 파일·사진·음성 메시지, 읽음·접속·입력 중 표시, 코치·레슨 상품, 정기권·쿠폰·포인트
- 외부 예약 시스템 자동 동기화
...
| 모집자가 입력 | 현재 Core: 제목, 모집 인원, 원하는 상대, 플레이 목적, 초보자 환영, 짧은 소개, 오픈채팅 링크. `IN_APP_CHAT` Match 출시 뒤 새 Match에는 링크를 입력하지 않음 |
...
- 운영자는 참가자의 프로필 스냅샷·신청 메시지·현재 연락 수단(오픈채팅 링크 또는 Match 채팅)을 조회하지 않는다.
...
| --- | --- | --- |
| 유료 Slot으로 세션 개설 | `1인 참가비`, 모집자도 결제한다는 안내, 15분 결제 기한, `결제 상태를 확인 중이에요` | `코트 예약`·운영자 승인 CTA를 만들지 않음 |
| 참가자 결제 초대 | 고정 참가비, 결제 기한, 결제 완료 뒤 참가 확정 | 초대 대기자를 `ACCEPTED` 또는 연락 가능으로 표시하지 않음 |
| 공급 철회 취소 | 안전한 취소 안내, `환불을 진행하고 있어요`, 최종 환불 상태 | 운영자 연락처·수기 이체·원문 철회 사유를 노출하지 않음 |
| 운영자 Commerce | PG 온보딩 상태, 플랫폼 수수료 무료 기간 종료 시각, 정산 요약 | 참가 신청 승인·참가자의 카드/계좌/상세 결제 오류를 제공하지 않음 |

공통 원칙은 다음과 같다.

- 유료 Slot에는 Pilot의 참고용 `예상 1인 비용` 대신 공개 전에 고정된 `1인 참가비`를 보여 준다.
- PG 리다이렉트 복귀만으로 결제 완료를 단정하지 않고, 서버가 승인 상태를 확인하는 로딩·오류 화면을 둔다.
- 참가자에게 별도 플랫폼 수수료를 더하지 않는다. 참가비·결제 기한·확정 조건·환불 기준을 짧은 문장으로 함께 보인다.
