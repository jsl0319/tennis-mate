...
| 구현 범위 | 별도 승인 전에는 Core MVP에 포함하지 않음 |

이 문서는 코트 운영자가 예약 가능한 코트 시간대를 공급하고, 사용자가 해당 시간대를 요청하여 코트 예약을 확정하는 후속 단계의 화면을 정의한다.

`03-screen-spec.md`의 Core MVP 화면과 구현 범위를 분리하기 위한 문서다. 이 문서가 존재한다는 이유만으로 운영자, 제휴 예약, 결제 또는 정산 기능을 현재 구현 범위로 간주하지 않는다.

## 2. 제품 단계

### 2.1 Court Partner Pilot

소수의 운영자와 코트 예약 수요 및 운영 가능성을 검증한다.
...
| 사업자 정보 | 정책에 따라 필수 | 민감 정보로 안전하게 처리 |
| 담당자명 | 필수 | 일반 사용자에게 비공개 |
| 담당자 연락처 | 필수 | 운영 연락 용도 |
| 코트 주소 | 필수 | 실제 운영 장소 |
| 운영 권한 증빙 | 필수 후보 | 최소 수집 원칙과 보존 기간 필요 |
| 운영 시간 | 선택 | 코트 등록 단계에서 보완 가능 |

증빙 파일은 공개 URL로 저장하거나 분석 이벤트에 포함하지 않는다.
...
| 승인 | 운영자 등록이 완료됐어요 | 운영자 홈으로 |
| 반려 | 현재 정보로는 등록이 어려워요 | 사유 확인·문의 |

반려 사유는 내부 위험 판단 정보를 과도하게 노출하지 않으면서 사용자가 수정 가능한 내용을 구체적으로 안내한다.
