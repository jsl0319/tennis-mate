...
첫 CP01 소스 구현은 `/partner-sessions`에서 공개 Slot을 시간순으로 보여 준다. 카드의 기본 일러스트는 운영자 사진 업로드가 열리기 전의 공통 코트 이미지이며, 사진이 없다는 사실을 예약 가능 여부나 서비스 보증으로 해석하게 하지 않는다. 카드 상세는 `/partner-sessions/{slotId}`에서 같은 안전한 공급 상태·코트·시각·비용·현장 정원을 다시 보여 준다. `ALLOCATED` 카드만 연결된 Match 상세로 이동하고, `AVAILABLE` 카드만 CP03으로 이동한다.
...
- 현재 수직 단위에서는 사진 업로드를 제공하지 않고 동일한 기본 코트 일러스트를 사용한다. 승인된 운영자 사진 업로드는 비공개 객체 보관·보존 기간·신고 절차가 확정된 다음 수직 단위에서만 열며, 외부 웹·지도에서 자동으로 사진을 가져오지 않는다.
...
- 운영자 사진 업로드의 보관 기간·삭제·신고 절차와 Pilot 포함 시점
