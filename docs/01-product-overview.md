...
Tennis Mate는 사용자가 상대방의 실력과 기대하는 플레이를 쉽게 이해하도록 돕는다. 또한 코트를 이미 예약한 사용자가 참가자를 모집할 수 있도록 하고, 제휴 코트 운영자가 준비한 시간에 일반 모집자가 세션을 열어 참가자를 모을 수 있게 하여 초보자가 장소와 상대를 각각 찾아야 하는 부담을 줄인다.
...
반대로 외부에서 코트를 먼저 예약한 사용자는 정해진 시간까지 참가자를 모집해야 한다. Tennis Mate는 **이미 예약된 코트와 참가자를 연결하는 흐름**과 제휴 코트 운영자가 준비한 시간에 일반 모집자가 세션을 여는 흐름을 함께 제공한다. 일반 사용자가 제휴 코트에 예약 요청을 보내거나 운영자가 참가자를 승인하지 않으며, 앱 내 결제와 정산은 별도 단계로 유지한다.
...
| 제휴 코트 | 운영자가 준비한 시간에 일반 모집자가 연 매칭 세션 | `Tennis Mate에서 준비한 코트예요` | 필수 Court Partner Pilot |

직접 예약 코트는 모집자가 제공한 정보를 기반으로 하며 Tennis Mate가 예약 사실을 검증한 것으로 표현하지 않는다. 제휴 코트 세션은 승인된 운영자가 준비한 시간에 연결된 Match로만 표시한다.
...
7. 모집자의 신청자 프로필 확인
8. 신청 수락
9. 현재 Core에서는 수락된 참가자에게 매칭별 카카오 오픈채팅 링크 공개. 서비스 내 Match 채팅 출시 뒤 새 Match는 앱 안의 채팅방 공개
10. 매칭 성사
...
유료 제휴 코트 세션은 운영자가 현장에 없어도 참가비를 확인할 수 있어야 한다. 이를 위해 Tennis Mate가 카드나 계좌 정보를 보관하거나 수기 이체를 대신 받는 대신, 운영자가 PG의 판매자/하위가맹점 온보딩을 완료하고 Tennis Mate는 세션·결제 상태를 중개하는 구조를 우선 검토한다. 실제 계약상 판매자·환불·세무·지급 책임은 PG 계약과 법무·세무 검토 후에만 확정한다.

사용자가 확정한 제품 정책은 운영자 부담 플랫폼 수수료 5%, 운영자 부담 PG 수수료, 그리고 운영자별 첫 성공 유료 결제 승인일부터 30일간 플랫폼 수수료 0%다. 참가자는 하나의 고정 참가비만 보며, 수수료를 별도로 더해 결제하지 않는다. 이 정책은 기존 Pilot 결제 없는 흐름을 바꾸지 않으며, 세부 모델·환불 미결 사항·구현 게이트는 `07-court-commerce-design.md`에서 관리한다.

### 14.2 서비스 내 Match 채팅

수락 뒤 일정과 준비물을 조율하는 대화는 외부 오픈채팅이 아니라 서비스 안의 Match 전용 텍스트 방으로 전환할 수 있다. 방에는 모집자와 실제 수락 참가자만 입장하며, 코트 운영자·대기 신청자·제3자는 입장하지 않는다. 채팅은 매칭과 분리된 개인 메신저가 아니므로 사용자 검색, DM, 사진·파일, 읽음·접속·입력 중 표시를 제공하지 않는다.
...
- 현재 Core에서는 모집자가 매칭별 카카오 오픈채팅 링크를 등록하고 모집자와 수락자에게만 공개한다. 서비스 내 Match 채팅 출시 뒤 새 Match의 연락은 `08-in-app-match-chat-design.md`의 방 멤버십 정책을 따른다.
...
- 제휴 코트 운영자 인증 방식
- 제휴 코트 세션 모집자·참가자 취소, 우천·노쇼의 환불·지원 정책
- 후기와 노쇼 정책
