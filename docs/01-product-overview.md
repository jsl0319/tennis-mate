...
Tennis Mate는 사용자가 상대방의 실력과 기대하는 플레이를 쉽게 이해하도록 돕는다. 또한 코트를 이미 예약한 사용자가 참가자를 모집할 수 있도록 하고, 제휴 코트의 예약 가능 시간대를 서비스 안에서 요청할 수 있게 하여 초보자가 장소와 상대를 각각 찾아야 하는 부담을 줄인다.
...
- 비어 있는 코트 시간대를 더 효율적으로 판매하고 싶음
- 전화, 메시지와 수기 장부로 예약 요청을 관리하고 있음
- 예약 요청, 승인과 취소 상태를 한곳에서 확인하고 싶음
- 초보자 고객에게 코트 위치, 이용 규칙과 준비물을 정확하게 안내하고 싶음
- 결제와 정산은 예약 운영이 검증된 이후 단계적으로 도입하기를 원함

코트 운영자는 테니스 실력을 평가하거나 매칭 참가자를 선택하는 역할이 아니다. 운영자는 코트 예약을 승인하고, 매칭 모집자가 참가 신청을 별도로 검토한다.
...
초보자는 해당 조건의 의미를 정확히 알지 못하거나 자신이 조건에 해당하는지 판단하기 어렵다. Tennis Mate는 초보자가 자신의 실제 경험에 맞춰 선택할 수 있는 쉬운 표현을 사용한다.
...
반대로 외부에서 코트를 먼저 예약한 사용자는 정해진 시간까지 참가자를 모집해야 한다. Tennis Mate는 **이미 예약된 코트와 참가자를 연결하는 흐름**과 제휴 코트 운영자가 예약 가능한 시간대를 직접 공급하는 흐름을 함께 제공한다. 제휴 코트 예약은 Core MVP 다음의 필수 확장 단계로 구현하며, 앱 내 결제와 정산은 별도 단계로 유지한다.

## 6. 문제 해결 접근법

Tennis Mate는 정확한 실력 점수를 측정하는 대신 **서로의 기대치를 맞추는 것**에 집중한다.
...
Tennis Mate는 코트 확보 경로를 다음과 같이 구분한다.

| 코트 유형 | 설명 | 사용자 표시 | 제품 단계 |
| --- | --- | --- | --- |
| 직접 예약 코트 | 모집자가 외부 사이트, 전화 등으로 예약한 코트 | `모집자가 코트를 예약했어요` | Core MVP |
| 제휴 코트 | 운영자가 등록한 시간대를 앱에서 요청하고 예약한 코트 | `Tennis Mate에서 예약이 확인된 코트예요` | 필수 Court Partner Pilot |

직접 예약 코트는 모집자가 제공한 정보를 기반으로 하며 Tennis Mate가 예약 사실을 검증한 것으로 표현하지 않는다. 제휴 코트만 서비스 내 예약 상태에 따라 확인된 코트로 표시한다.

Core MVP에서는 모집자가 코트의 이름, 위치, 예약 일시, 이용 시간과 전체 비용을 입력한다. 예상 1인 비용은 모집자를 포함한 예상 총 참여 인원으로 나눈 뒤 1원 단위 올림하여 자동 표시하고 사용자가 수정할 수 없다. 참가자 간 비용 정산은 앱 밖에서 진행한다.
...
| 코트 운영 효율 | 운영자는 예약 가능한 시간대와 요청 상태를 한곳에서 관리한다. Court Partner Pilot 이후 가치 |
...
### 8.7 코트 예약의 출처를 투명하게 보여준다

모집자가 직접 예약한 코트와 서비스에서 예약 상태를 확인한 제휴 코트를 구분한다. 사용자가 코트 예약의 확인 주체와 비용 처리 방식을 오해하지 않도록 문구와 배지를 다르게 사용한다.
...
7. 모집자의 신청자 프로필 확인
8. 신청 수락
9. 수락된 참가자에게 매칭별 카카오 오픈채팅 링크 공개
10. 매칭 성사
...
Tennis Mate의 차별점은 매칭 기능 자체가 아니다. 기존 서비스도 사람을 연결할 수 있다.
...
이 기능들은 필요성이 실제 사용자 행동과 피드백으로 확인된 뒤 검토한다.
...
예약 수요와 운영 가능성이 확인된 후 승인 후 결제, 결제 제한 시간, 취소·환불과 운영자 정산을 추가한다. 운영자는 코트 예약을 승인하고, 매칭 모집자는 함께 칠 참가자를 승인한다. 두 책임을 하나의 승인으로 합치지 않는다.

Court Partner Pilot의 운영자 및 사용자 예약 화면은 `03-1-court-partner-screen-spec.md`에서 별도로 정의한다. 이 문서는 Core MVP 구현 범위와 혼동되지 않도록 분리한다.

운영자는 초대 없이 직접 신청할 수 있다. 다만 자동 확인은 운영 권한의 최종 보증이 아니라 위험을 분류하는 수단이다. 사업자 정상 여부, 표준화한 사업장 주소와 테니스장 장소 정보의 일치, 이미 승인된 동일 장소 운영자 부재를 모두 충족한 경우에만 공개 권한을 자동 부여한다. 이외의 불일치·공공시설·지점·위탁운영·중복 장소 사례는 증빙과 운영 검토를 거쳐 공개한다. 자동 확인이 끝난 운영자는 비공개 코트·시간대 초안을 만들 수 있지만, 공개 권한이 없으면 예약 요청을 받을 수 없다.
...
16. 운영자 예약 승인과 앱 결제는 Court Partner Pilot 이후 단계로 분리한다.
...
- 모집자가 매칭별 카카오 오픈채팅 링크를 등록하고 모집자와 수락자에게만 공개한다.
...
- 제휴 코트 운영자 인증 방식
- 제휴 코트 예약 승인, 결제 제한 시간, 환불 및 정산 정책
- 후기와 노쇼 정책
