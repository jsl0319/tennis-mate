...
수락 뒤 일정과 준비물을 조율하는 대화는 모든 Match에서 서비스 안의 Match 전용 텍스트 방으로 처리한다. 방에는 모집자와 실제 수락 참가자만 입장하며, 코트 운영자·대기 신청자·제3자는 입장하지 않는다. 채팅 탭은 내가 입장 권한을 가진 방만 `내가 만든 매칭`과 `내가 신청한 매칭`으로 나누어 보여 주며, 새 방 만들기·사용자 검색은 제공하지 않는다. 채팅은 매칭과 분리된 개인 메신저가 아니므로 사용자 검색, DM, 사진·파일, 읽음·접속·입력 중 표시를 제공하지 않는다.
