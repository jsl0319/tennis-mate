...
Tennis Mate는 사용자가 상대방의 실력과 기대하는 플레이를 쉽게 이해하도록 돕는다. 또한 코트를 이미 예약한 사용자가 참가자를 모집할 수 있도록 하여 초보자가 장소와 상대를 각각 찾아야 하는 부담을 줄인다.
...
반대로 외부에서 코트를 먼저 예약한 사용자는 정해진 시간까지 참가자를 모집해야 한다. Tennis Mate는 우선 **이미 예약된 코트와 참가자를 연결하는 흐름**을 제공하고, 이후에는 제휴 코트 운영자가 예약 가능한 시간대를 직접 공급하는 구조로 확장한다.
...
| 제휴 코트 | 운영자가 등록한 시간대를 앱에서 요청하고 예약한 코트 | `Tennis Mate에서 예약이 확인된 코트예요` | Court Partner Pilot 이후 |
