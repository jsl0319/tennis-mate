...
export function ProfileEditPage() {
  ...
  if (loading) return <main className="grid min-h-svh place-items-center bg-[var(--tm-bg-page)] text-sm text-[var(--tm-text-secondary)]">프로필을 불러오는 중이에요…</main>;
  ...
  return <main className="min-h-svh bg-[var(--tm-bg-page)] px-5 pb-40 pt-5 text-[var(--tm-text-primary)]"><section className="mx-auto max-w-[560px]"><header><BackButton className="inline-flex size-11 items-center justify-center rounded-full text-xl" fallbackPath="/my" /><p className="mt-6 text-sm font-semibold text-[var(--tm-action-primary)]">내 테니스 이야기</p><h1 className="mt-1 text-2xl font-bold leading-tight">지금의 나에게 맞게<br />프로필을 다듬어요</h1><p className="mt-3 text-sm leading-6 text-[var(--tm-text-secondary)]">저장하면 이후 추천에 반영돼요.</p></header><section className="mt-7 rounded-3xl border border-[var(--tm-border-default)] bg-white p-5 shadow-[0_4px_14px_rgba(49,94,158,0.05)]"><h2 className="text-sm font-semibold text-[var(--tm-action-primary)]">활동 지역</h2><p className="mt-2 text-sm leading-6 text-[var(--tm-text-secondary)]">자주 테니스를 치고 싶은 곳을 골라 주세요.</p><div className="mt-4 grid grid-cols-2 gap-3"><label className="text-sm font-semibold">시·도<select className="mt-2 h-12 w-full rounded-xl border border-[var(--tm-border-default)] bg-white px-3 font-normal" onChange={(event) => void selectCity(event.target.value)} value={draft.cityCode}><option value="">선택</option>{cities.map((city) => <option key={city.code} value={city.code}>{city.shortName ?? city.name}</option>)}</select></label><label className="text-sm font-semibold">시·군·구<select className="mt-2 h-12 w-full rounded-xl border border-[var(--tm-border-default)] bg-white px-3 font-normal" onChange={(event) => setDraft((current) => ({ ...current, regionCode: event.target.value }))} value={draft.regionCode}><option value="">선택</option>{districts.map((district) => <option key={district.code} value={district.code}>{district.name}</option>)}</select></label></div><label className="mt-4 flex min-h-11 items-center gap-3 text-sm"><input checked={draft.nearbyRegionAllowed} className="size-5 accent-[var(--tm-action-primary)]" onChange={(event) => setDraft((current) => ({ ...current, nearbyRegionAllowed: event.target.checked }))} type="checkbox" />선택 지역과 가까운 시·군·구도 괜찮아요</label></section><ProfileQuestion description="정확하지 않아도 괜찮아요." onSelect={(value) => setDraft((current) => ({ ...current, experienceRange: value as ExperienceRange }))} options={experienceOptions} selected={draft.experienceRange} title="테니스와 친해진 지" /><ProfileQuestion description="가장 가까운 하나를 골라 주세요." onSelect={(value) => setDraft((current) => ({ ...current, rallyLevel: value as RallyLevel }))} options={rallyOptions} selected={draft.rallyLevel} title="요즘 랠리는" /><ProfileQuestion description="게임 실력을 평가하는 질문이 아니에요." onSelect={(value) => setDraft((current) => ({ ...current, gameExperience: value as GameExperience }))} options={gameOptions} selected={draft.gameExperience} title="게임 경험" /><ProfileQuestion description="지금 원하는 플레이를 최대 2개 골라 주세요." onSelect={(value) => togglePurpose(value as PlayPurpose)} options={purposeOptions} selected={draft.playPurposes} title="원하는 플레이" />{error ? <p className="mt-4 rounded-2xl bg-[var(--tm-status-error-bg)] px-4 py-3 text-sm leading-6 text-[var(--tm-status-error-text)]">{error}</p> : null}</section><footer className="fixed inset-x-0 bottom-0 z-30 border-t border-[var(--tm-border-default)] bg-white/95 px-5 pb-[max(16px,env(safe-area-inset-bottom))] pt-3 backdrop-blur"><div className="mx-auto max-w-[560px]"><p className="text-center text-xs leading-5 text-[var(--tm-text-secondary)]">이미 보낸 신청에는 신청 당시 프로필이 유지돼요.</p><button className="mt-2 min-h-[52px] w-full rounded-2xl bg-[var(--tm-action-primary)] px-4 text-sm font-semibold text-white shadow-[0_8px_20px_rgba(49,94,158,0.18)] disabled:opacity-45" disabled={!valid || saving} onClick={() => void save()} type="button">{saving ? "저장 중…" : "저장하기"}</button></div></footer></main>;
}
