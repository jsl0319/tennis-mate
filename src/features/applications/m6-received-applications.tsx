...
function EmptyHostedMatches() {
  return <section className="mt-10 rounded-3xl border border-dashed border-[var(--tm-border-strong)] bg-white px-5 py-10 text-center"><p className="text-2xl">🎾</p><h2 className="mt-4 font-bold">아직 만든 매칭이 없어요</h2><p className="mt-2 text-sm leading-6 text-[var(--tm-text-secondary)]">코트 예약 전에도 일정과 지역을 정해 메이트를 모집할 수 있어요.</p><Link className="mt-5 inline-flex min-h-11 items-center rounded-2xl bg-[var(--tm-action-primary)] px-4 text-sm font-semibold text-white" href="/matches/new">매칭 만들기</Link></section>;
}
...
function HostedMatchCard({ match, onChanged }: { match: HostedMatch; onChanged: () => Promise<void> }) {
  ...
  return <>
    <section className="rounded-3xl border border-[var(--tm-border-default)] bg-white p-5 shadow-[0_4px_14px_rgba(49,94,158,0.05)]">
      ...
      {match.canClose ? <button className="mt-3 min-h-11 w-full rounded-2xl border border-[var(--tm-border-default)] px-4 text-sm font-semibold text-[var(--tm-text-muted)] disabled:opacity-50" disabled={action !== null} onClick={() => setConfirmAction("close")} type="button">모집 마감</button> : null}
