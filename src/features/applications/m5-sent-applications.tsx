...
export function M5SentApplications() {
  ...
  const withdraw = async (applicationId: string) => {
    setWithdrawingId(applicationId); setError("");
    try {
      const response = await fetch(`/api/v1/applications/${encodeURIComponent(applicationId)}/withdraw`, { method: "POST" });
      const body: unknown = await response.json();
      if (!response.ok) throw new Error(typeof body === "object" && body !== null && "error" in body && typeof body.error === "object" && body.error !== null && "message" in body.error && typeof body.error.message === "string" ? body.error.message : "신청을 철회하지 못했어요.");
      await load();
  ...
  return <main className="min-h-svh bg-[#fffdfc] px-5 pb-10 pt-6 text-[#1a221e]"><div className="mx-auto max-w-[560px]"><Link aria-label="홈으로 돌아가기" className="inline-flex size-11 items-center justify-center rounded-full text-xl" href="/">←</Link><p className="mt-5 text-sm font-semibold text-[#1f7a55]">같이 치기</p><h1 className="mt-1 text-2xl font-bold">보낸 신청</h1><p className="mt-2 text-sm leading-6 text-[#5c6b63]">모집자의 확인을 기다리고 있어요.</p>
    {error && items !== null ? <div className="mt-5 rounded-2xl bg-[#fff1ef] px-4 py-3 text-sm text-[#a13d32]">{error}</div> : null}
    {error && items === null ? <div className="mt-8 rounded-3xl border border-[#d8e0db] bg-white p-5"><p>{error}</p><button className="mt-4 rounded-2xl bg-[#1f7a55] px-4 py-3 text-sm font-semibold text-white" onClick={() => void load()} type="button">다시 불러오기</button></div> : items === null ? <p className="mt-12 text-center text-sm text-[#5c6b63]">신청 내역을 불러오는 중이에요…</p> : items.length === 0 ? <section className="mt-10 rounded-3xl border border-dashed border-[#c7d6ce] bg-white px-5 py-10 text-center"><p className="text-2xl">🎾</p><h2 className="mt-4 font-bold">아직 보낸 신청이 없어요</h2><p className="mt-2 text-sm leading-6 text-[#5c6b63]">마음에 드는 매치를 찾아 부담 없이 신청해 보세요.</p><Link className="mt-5 inline-flex min-h-11 items-center rounded-2xl bg-[#1f7a55] px-4 text-sm font-semibold text-white" href="/">매치 찾아보기</Link></section> : <div className="mt-7 space-y-3">{items.map((item) => <article className="rounded-3xl border border-[#d8e0db] bg-white p-5" key={item.id}><Link className="block transition-colors hover:text-[#1f7a55]" href={`/matches/${item.match.id}`}><div className="flex items-start justify-between gap-3"><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${item.status === "PENDING" || item.status === "ACCEPTED" ? "bg-[#eff9f4] text-[#1f7a55]" : "bg-[#f3f5f4] text-[#5c6b63]"}`}>{item.statusLabel}</span><span className="text-xs text-[#5c6b63]">{appliedDate(item.createdAt)}</span></div><h2 className="mt-4 text-lg font-bold">{item.match.title}</h2><p className="mt-3 text-sm text-[#405047]">🗓 {schedule(item.match.startsAt)}</p><p className="mt-1 text-sm text-[#405047]">📍 {item.match.courtName ?? "코트는 함께 정해요"} · {item.match.regionName}</p><p className="mt-3 text-sm font-semibold text-[#1f7a55]">{item.match.estimatedFeePerPersonKrw === null ? "코트와 비용을 함께 정해요" : `예상 1인 약 ${item.match.estimatedFeePerPersonKrw.toLocaleString("ko-KR")}원`}</p><p className="mt-4 border-t border-[#edf0ee] pt-3 text-sm font-medium leading-6 text-[#405047]">{nextStepMessage(item.status)}</p>{item.message ? <p className="mt-2 text-sm leading-6 text-[#5c6b63]">“{item.message}”</p> : null}</Link>{item.contact ? <><p className="mt-4 rounded-2xl bg-[#eff9f4] px-4 py-3 text-sm leading-6 text-[#315b45]">수락된 참가자끼리 오픈채팅에서 코트와 비용을 조율해요.</p><a className="mt-3 flex min-h-[52px] items-center justify-center rounded-2xl bg-[#1f7a55] px-4 text-center text-sm font-semibold text-white" href={item.contact.url} rel="noreferrer" target="_blank">{item.contact.label}</a></> : null}{item.status === "PENDING" ? <button className="mt-3 min-h-11 w-full rounded-2xl border border-[#d8e0db] px-4 text-sm font-semibold text-[#405047] disabled:opacity-50" disabled={withdrawingId === item.id} onClick={() => setWithdrawConfirmId(item.id)} type="button">신청 철회</button> : null}</article>)}</div>}
