...
export function InternalMatchChatReportReview() {
  ...
  return <main className="min-h-svh bg-[var(--tm-bg-page)] px-5 pb-12 pt-11 text-[var(--tm-text-primary)]"><div className="mx-auto max-w-[390px]">
    ...
    <div className="-mx-5 mt-5 flex gap-2 overflow-x-auto px-5 pb-1" role="tablist" aria-label="채팅 신고 상태">
      {statusTabs.map((tab) => <button aria-selected={status === tab.value} className={`min-h-10 shrink-0 rounded-full px-4 text-sm font-semibold ${status === tab.value ? "bg-[var(--tm-action-primary)] text-white" : "border border-[var(--tm-border-default)] bg-white text-[var(--tm-text-secondary)]"}`} key={tab.value} onClick={() => { if (status === tab.value) { void load(); return; } setReports(null); setStatus(tab.value); }} role="tab" type="button">{tab.label}</button>)}
    </div>
