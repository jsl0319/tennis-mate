...
export function OperatorApplicationFlow() {
  ...
  if (screen === "loading") return <main className="min-h-svh bg-[#fffdfc] px-5 pt-11 text-sm text-[#5c6b63]"><div className="mx-auto max-w-[390px]">운영자 등록을 준비하고 있어요…</div></main>;
...
function Shell({ children }: { children: React.ReactNode }) {
  return <main className="min-h-svh bg-[#fffdfc] px-5 pb-28 pt-11 text-[#1a221e]"><div className="mx-auto max-w-[390px]">{children}</div></main>;
}

function Intro({ onStart }: { onStart: () => void }) {
  return <Shell><div className="flex items-center justify-between text-sm text-[#5c6b63]"><span>운영자 등록</span><span>시작하기</span></div><div className="mt-[72px]"><p className="text-sm text-[#1f7a55]">COURT PARTNER PILOT</p><h1 className="mt-2 text-2xl font-bold leading-[34px]">테니스장 운영자이신가요?</h1><p className="mt-2 text-sm leading-6 text-[#5c6b63]">사업자와 테니스장 정보를 확인한 뒤,<br />준비한 시간대를 이용자에게 공개할 수 있어요.</p></div><section className="mt-5 rounded-2xl border border-[#d8e0db] bg-white p-5"><h2 className="text-lg font-medium">등록 전에 이렇게 확인해요</h2><ol className="mt-3 space-y-3">{[["사업자 정보", "등록번호와 개업일을 확인해요."], ["테니스장 정보", "주소와 운영 권한을 함께 살펴봐요."], ["공개 준비", "승인 후 시간대를 공개할 수 있어요."]].map(([title, description], index) => <li className="flex gap-3" key={title}><span className="flex size-7 shrink-0 items-center justify-center rounded-full bg-[#eff9f4] text-sm text-[#1f7a55]">{index + 1}</span><span className="text-sm leading-5"><strong className="block font-normal">{title}</strong><span className="text-[#5c6b63]">{description}</span></span></li>)}</ol></section><Notice className="mt-4">심사 중에는 코트와 시간대를 초안으로 저장할 수 있어요. 이용자에게는 아직 보이지 않아요.</Notice><button className="fixed inset-x-5 bottom-7 mx-auto flex h-14 w-[calc(100%-40px)] max-w-[350px] items-center justify-center rounded-2xl bg-[#1f7a55] text-lg font-medium text-white" onClick={onStart} type="button">운영자 등록 시작하기</button></Shell>;
}
...
function BusinessStep({ draft, error, onBack, onNext, set }: { draft: Draft; error: string; onBack: () => void; onNext: () => void; set: <Key extends keyof Draft>(key: Key, value: Draft[Key]) => void }) {
  return <Shell><Progress current={1} onBack={onBack} /><p className="mt-8 text-sm text-[#1f7a55]">COURT PARTNER PILOT</p><h1 className="mt-2 text-2xl font-bold leading-[34px]">사업자 정보를 알려주세요</h1><p className="mt-2 text-sm leading-6 text-[#5c6b63]">운영자 등록을 위해 필요한 정보예요.<br />확인에 실패해도 바로 공개되지는 않아요.</p><div className="mt-5 space-y-3"><Field label="사업자명" value={draft.businessName} placeholder="예) 마포 테니스파크" onChange={(value) => set("businessName", value)} /><Field label="사업자등록번호" inputMode="numeric" value={draft.businessRegistrationNumber} placeholder="숫자 10자리" onChange={(value) => set("businessRegistrationNumber", value)} /><Field label="개업일" type="date" value={draft.businessOpenedOn} placeholder="YYYY.MM.DD" onChange={(value) => set("businessOpenedOn", value)} /><Field label="대표자명" value={draft.representativeName} placeholder="사업자등록증 기준 이름" onChange={(value) => set("representativeName", value)} /></div><Notice className="mt-6">사업자등록번호는 확인 목적으로만 사용해요.<br />등록 정보를 다른 이용자에게 공개하지 않아요.</Notice><StepError error={error} /><button className="fixed inset-x-5 bottom-7 mx-auto flex h-14 w-[calc(100%-40px)] max-w-[350px] items-center justify-center rounded-2xl bg-[#1f7a55] text-lg font-medium text-white" onClick={onNext} type="button">다음</button></Shell>;
}
...
function VenueStep({ draft, error, submitting, onBack, onSubmit, set }: { draft: Draft; error: string; submitting: boolean; onBack: () => void; onSubmit: () => void; set: <Key extends keyof Draft>(key: Key, value: Draft[Key]) => void }) {
  return <Shell><Progress current={2} onBack={onBack} /><p className="mt-8 text-sm text-[#1f7a55]">COURT PARTNER PILOT</p><h1 className="mt-2 text-2xl font-bold leading-[34px]">테니스장 정보를 알려주세요</h1><p className="mt-2 text-sm leading-6 text-[#5c6b63]">직접 운영하는 테니스장만 등록할 수 있어요.<br />정보가 맞지 않으면 확인 자료를 요청할 수 있어요.</p><div className="mt-5 space-y-3"><Field label="테니스장 이름" value={draft.venueName} placeholder="예) 마포 테니스파크" onChange={(value) => set("venueName", value)} /><Field label="테니스장 주소" value={draft.venueAddress} placeholder="주소 검색으로 정확히 입력해 주세요" onChange={(value) => set("venueAddress", value)} /><Field label="운영자 연락처" inputMode="tel" value={draft.operatorPhone} placeholder="심사 결과를 받을 휴대폰 번호" onChange={(value) => set("operatorPhone", value)} /><div><p className="text-sm leading-5">운영 권한 확인 자료 <span className="text-[#5c6b63]">(필요 시)</span></p><div className="mt-1 flex h-[58px] items-center rounded-xl border border-[#d8e0db] bg-white px-3 text-sm text-[#5c6b63]">검토가 필요하면 안전한 제출 방법을 안내해 드려요.</div></div></div><Notice className="mt-6">사업자 정보와 장소 정보가 다를 때만<br />임대차 계약서나 위임장 등을 요청할 수 있어요.</Notice><StepError error={error} /><button className="fixed inset-x-5 bottom-7 mx-auto flex h-14 w-[calc(100%-40px)] max-w-[350px] items-center justify-center rounded-2xl bg-[#1f7a55] text-lg font-medium text-white disabled:opacity-50" disabled={submitting} onClick={onSubmit} type="button">{submitting ? "등록 요청 중…" : "등록 요청하기"}</button></Shell>;
}

function Progress({ current, onBack }: { current: 1 | 2; onBack: () => void }) { return <><div className="flex items-center justify-between"><button aria-label="이전 단계로 돌아가기" className="-ml-2 min-h-11 min-w-11 text-2xl font-bold" onClick={onBack} type="button">‹</button><span className="text-sm text-[#5c6b63]">{current} / 2</span></div><div aria-label={`${current} / 2 단계`} className="mt-5 h-1 overflow-hidden rounded-full bg-[#eff9f4]"><div className="h-full rounded-full bg-[#1f7a55]" style={{ width: current === 1 ? "50%" : "100%" }} /></div></> }
function Field({ label, value, placeholder, onChange, type = "text", inputMode }: { label: string; value: string; placeholder: string; onChange: (value: string) => void; type?: "text" | "date"; inputMode?: React.HTMLAttributes<HTMLInputElement>["inputMode"] }) { const id = `operator-${label}`; return <label className="block text-sm leading-5" htmlFor={id}>{label}<input className="mt-1 h-12 w-full rounded-xl border border-[#d8e0db] bg-white px-3 text-sm outline-none transition focus:border-[#1f7a55]" id={id} inputMode={inputMode} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} type={type} value={value} /></label>; }
function Notice({ children, className = "" }: { children: React.ReactNode; className?: string }) { return <div className={`rounded-xl bg-[#eff9f4] p-4 text-sm leading-5 text-[#5c6b63] ${className}`}>{children}</div>; }
function StepError({ error }: { error: string }) { return error ? <p className="mt-3 text-sm leading-5 text-[#a13d32]" role="alert">{error}</p> : null; }
