...
function DetailAction({ detail, onApply }: { detail: Detail; onApply: () => void }) {
  ...
  if (detail.viewer.relation === "APPLICANT" && detail.viewer.applicationStatus === "ACCEPTED" && detail.contact) return <a className="flex min-h-[52px] w-full items-center justify-center rounded-3xl bg-[var(--tm-action-primary)] font-semibold text-white" href={detail.contact.url} rel="noreferrer" target="_blank">{detail.contact.label}</a>;
