...
function StepThree({ fee, form, set }: { fee: number; form: { courtSource: CourtSource; recruitCount: number; totalCourtFeeKrw: string; additionalCostNote: string; introduction: string }; set: (key: "totalCourtFeeKrw" | "additionalCostNote" | "introduction", value: string) => void }) {
  return <div className="mt-8"><p className="text-sm font-semibold text-[var(--tm-action-primary)]">마지막 확인</p><h1 className="mt-1 text-2xl font-bold">비용과 안내를 알려주세요</h1>{form.courtSource === "EXTERNAL_RESERVED" ? <Fields><label>전체 코트 비용<input min="0" onChange={(event) => set("totalCourtFeeKrw", event.target.value)} type="number" value={form.totalCourtFeeKrw} /></label><div className="rounded-2xl bg-[var(--tm-bg-subtle)] p-4 text-sm leading-6">예상 1인 비용 <strong>약 {fee.toLocaleString("ko-KR")}원</strong><br /><span className="text-[var(--tm-text-secondary)]">Tennis Mate에서는 이 비용을 결제하지 않아요.</span></div><label>추가 비용 안내 <span className="font-normal text-[var(--tm-text-secondary)]">(선택)</span><input onChange={(event) => set("additionalCostNote", event.target.value)} value={form.additionalCostNote} /></label></Fields> : <Notice />}<Fields><label>소개 <span className="font-normal text-[var(--tm-text-secondary)]">(선택)</span><textarea maxLength={300} onChange={(event) => set("introduction", event.target.value)} value={form.introduction} /></label><p className="rounded-2xl bg-[var(--tm-bg-subtle)] px-4 py-3 text-sm leading-6 text-[var(--tm-action-hover)]">수락된 참가자와 서비스 내 채팅에서 일정과 준비를 조율해요.</p></Fields></div>;
}
...
function Notice() { return <section className="mt-5 rounded-2xl bg-[var(--tm-bg-subtle)] p-4"><h2 className="font-semibold text-[var(--tm-action-primary)]">코트와 비용은 함께 정해요</h2><p className="mt-2 text-sm leading-6 text-[var(--tm-text-secondary)]">일정과 활동 지역만 먼저 정해 보세요. 수락된 참가자와 서비스 내 채팅에서 코트와 비용을 정해요.</p></section>; }
