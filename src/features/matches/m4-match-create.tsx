...
export function M4MatchCreate() {
  ...
  const [form, setForm] = useState(() => ({ clientRequestId: crypto.randomUUID(), date: "", time: "", duration: 120, cityCode: "", regionCode: "", courtName: "", address: "", courtNumber: "", title: "", recruitCount: 1, playPurposes: ["RALLY_PRACTICE"], partnerPreference: "COMPLETE_BEGINNER_WELCOME", totalCourtFeeKrw: "", additionalCostNote: "", introduction: "" }));
  ...
  const next = () => {
    ...
    if (step === 1 && (!form.courtName.trim() || !form.address.trim())) return setError("예약한 코트의 이름과 주소를 입력해 주세요.");
    ...
    if (step === 3 && (form.totalCourtFeeKrw === "" || Number(form.totalCourtFeeKrw) < 0)) return setError("전체 코트 비용을 0원 이상으로 입력해 주세요.");
  ...
  const submit = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/v1/matches", {
        ...
        body: JSON.stringify({
          clientRequestId: form.clientRequestId, title: form.title, startsAt, endsAt, regionCode: form.regionCode, courtSource: "EXTERNAL_RESERVED",
          externalCourt: { name: form.courtName, address: form.address, courtNumber: form.courtNumber || null, imageUploadId: courtImage?.uploadId ?? null },
          recruitCount: form.recruitCount, playPurposes: form.playPurposes, partnerPreference: form.partnerPreference,
          totalCourtFeeKrw: Number(form.totalCourtFeeKrw),
  ...
  return <main className="min-h-svh bg-[var(--tm-bg-page)] px-5 pb-28 pt-6 text-[var(--tm-text-primary)]"><section className="mx-auto max-w-[560px]"><header className="sticky top-0 z-10 -mx-5 border-b border-[var(--tm-border-subtle)] bg-[var(--tm-bg-page)]/95 px-5 pb-4 pt-1 backdrop-blur"><div className="flex items-center gap-3"><button aria-label={step === 1 ? "이전 화면으로 돌아가기" : "이전 단계"} className="grid size-11 place-items-center rounded-full text-xl" onClick={() => step === 1 ? router.replace("/") : setStep((current) => current - 1)} type="button">←</button><div className="flex-1"><div className="h-1 rounded-full bg-[var(--tm-border-default)]"><div className="h-full rounded-full bg-[var(--tm-action-primary)] transition-all" style={{ width: `${step * 25}%` }} /></div><p className="mt-1 text-right text-xs text-[var(--tm-text-secondary)]">{step}/4</p></div></div></header>
    ...
    {step === 1 ? <StepOne cities={cities} courtImage={courtImage} courtImageError={courtImageError} courtImageUploading={courtImageUploading} districts={districts} form={form} onCourtImageChange={(file) => void uploadCourtImage(file)} onSelectCity={(code) => void selectCity(code)} set={(key, value) => set(key, value as never)} /> : null}
...
function StepOne({ cities, courtImage, courtImageError, courtImageUploading, districts, form, onCourtImageChange, onSelectCity, set }: { cities: Region[]; courtImage: CourtImageDraft | null; courtImageError: string; courtImageUploading: boolean; districts: Region[]; form: { date: string; time: string; duration: number; cityCode: string; regionCode: string; courtName: string; address: string; courtNumber: string }; onCourtImageChange: (file: File | null) => void; onSelectCity: (code: string) => void; set: (key: "date" | "time" | "duration" | "cityCode" | "regionCode" | "courtName" | "address" | "courtNumber", value: string | number) => void }) {
  return <div className="mt-8"><p className="text-sm font-semibold text-[var(--tm-action-primary)]">매칭 만들기</p><h1 className="mt-1 text-2xl font-bold">예약한 코트에서<br />함께 칠 사람을 찾아요</h1><p className="mt-2 text-sm leading-6 text-[var(--tm-text-secondary)]">코트 정보와 비용을 먼저 알려 주세요.</p><section className="mt-5 rounded-2xl bg-[var(--tm-bg-subtle)] p-4"><h2 className="font-semibold text-[var(--tm-action-primary)]">아직 코트를 예약하지 않았나요?</h2><p className="mt-2 text-sm leading-6 text-[var(--tm-text-secondary)]">운영자가 준비한 시간으로 코트 매칭을 열 수 있어요.</p><Link className="mt-3 inline-flex min-h-11 items-center rounded-xl bg-white px-3 text-sm font-semibold text-[var(--tm-action-primary)] ring-1 ring-inset ring-[var(--tm-border-default)]" href="/partner-sessions">코트 매칭 둘러보기 →</Link></section><Fields><label>날짜<input min={new Date().toISOString().slice(0, 10)} onChange={(event) => set("date", event.target.value)} type="date" value={form.date} /></label><label>시작 시간<input onChange={(event) => set("time", event.target.value)} type="time" value={form.time} /></label><label>이용 시간<select onChange={(event) => set("duration", Number(event.target.value))} value={form.duration}><option value={60}>1시간</option><option value={90}>1시간 30분</option><option value={120}>2시간</option></select></label><label>시<select onChange={(event) => onSelectCity(event.target.value)} value={form.cityCode}><option value="">선택</option>{cities.map((city) => <option key={city.code} value={city.code}>{city.shortName ?? city.name}</option>)}</select></label><label>구<select onChange={(event) => set("regionCode", event.target.value)} value={form.regionCode}><option value="">선택</option>{districts.map((district) => <option key={district.code} value={district.code}>{district.name}</option>)}</select></label></Fields><Fields><label>코트장 이름<input onChange={(event) => set("courtName", event.target.value)} value={form.courtName} /></label><label>주소<input onChange={(event) => set("address", event.target.value)} value={form.address} /></label><label>코트 번호 <span className="font-normal text-[var(--tm-text-secondary)]">(선택)</span><input onChange={(event) => set("courtNumber", event.target.value)} value={form.courtNumber} /></label></Fields><CourtImageUpload courtImage={courtImage} error={courtImageError} isUploading={courtImageUploading} onChange={onCourtImageChange} /></div>;
}
...
function StepThree({ fee, form, set }: { fee: number; form: { recruitCount: number; totalCourtFeeKrw: string; additionalCostNote: string; introduction: string }; set: (key: "totalCourtFeeKrw" | "additionalCostNote" | "introduction", value: string) => void }) {
  return <div className="mt-8"><p className="text-sm font-semibold text-[var(--tm-action-primary)]">마지막 확인</p><h1 className="mt-1 text-2xl font-bold">비용과 안내를 알려주세요</h1><Fields><label>전체 코트 비용<input min="0" onChange={(event) => set("totalCourtFeeKrw", event.target.value)} type="number" value={form.totalCourtFeeKrw} /></label><div className="rounded-2xl bg-[var(--tm-bg-subtle)] p-4 text-sm leading-6">예상 1인 비용 <strong>약 {fee.toLocaleString("ko-KR")}원</strong><br /><span className="text-[var(--tm-text-secondary)]">Rally On에서는 이 비용을 결제하지 않아요.</span></div><label>추가 비용 안내 <span className="font-normal text-[var(--tm-text-secondary)]">(선택)</span><input onChange={(event) => set("additionalCostNote", event.target.value)} value={form.additionalCostNote} /></label></Fields><Fields><label>소개 <span className="font-normal text-[var(--tm-text-secondary)]">(선택)</span><textarea maxLength={300} onChange={(event) => set("introduction", event.target.value)} value={form.introduction} /></label><p className="rounded-2xl bg-[var(--tm-bg-subtle)] px-4 py-3 text-sm leading-6 text-[var(--tm-action-hover)]">수락된 참가자와 서비스 내 채팅에서 당일 준비를 조율해요.</p></Fields></div>;
}

function StepFour({ fee, form }: { fee: number; form: { date: string; time: string; title: string; recruitCount: number; courtName: string } }) {
  return <div className="mt-8"><p className="text-sm font-semibold text-[var(--tm-action-primary)]">공개 전 확인</p><h1 className="mt-1 text-2xl font-bold">이렇게 모집할까요?</h1><article className="mt-6 rounded-3xl border border-[var(--tm-border-default)] bg-white p-5 shadow-sm"><p className="text-sm font-semibold text-[var(--tm-action-primary)]">모집자가 코트를 예약했어요</p><h2 className="mt-3 text-xl font-bold">{form.title}</h2><p className="mt-4 text-sm leading-6 text-[var(--tm-text-muted)]">🗓 {form.date} · {form.time}<br />📍 {form.courtName}<br />👥 추가 {form.recruitCount}명 · 1인 약 {fee.toLocaleString("ko-KR")}원</p><p className="mt-4 border-t border-[var(--tm-border-subtle)] pt-3 text-xs leading-5 text-[var(--tm-text-secondary)]">코트 비용은 참가자끼리 별도로 정산해요.</p></article></div>;
}
