...
export function MatchCard({ match, returnTo = "/" }: { match: MatchCardData; returnTo?: string }) {
  return (
    <Link
      className="flex min-h-[218px] w-full gap-4 rounded-3xl border border-[#d8e0db] bg-white p-5 shadow-sm transition hover:border-[#79b99a] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#1f7a55]"
    ...
    >
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="flex min-h-5 items-center gap-1 overflow-hidden text-[11px] font-semibold leading-4">
          <span className="shrink-0 text-[#1f7a55]">{match.statusLabel}</span>
          {match.beginnerWelcome ? <span className="truncate text-[#6c5a18]">· 🌱 초보자 환영</span> : null}
        </div>
        <h2 className="line-clamp-2 min-h-[52px] text-lg font-medium leading-[26px]">{match.title}</h2>
        <p className="min-h-5 truncate text-sm leading-5 text-[#1f7a55]">
          {match.recommendationReasons[0]?.label ?? <span aria-hidden>&nbsp;</span>}
        </p>
        <p className="text-sm leading-5 text-[#5c6b63]">
          {formatSchedule(match.startsAt, match.endsAt)}<br />
          {match.region.name} · {match.court.name ?? "코트는 함께 정해요"}<br />
          <span className="text-[#405047]">{match.court.sourceLabel}</span><br />
...
export function MatchCardSkeleton() {
  return <div className="h-[218px] animate-pulse rounded-3xl bg-[#eaf0ec]" />;
}
